...
namespace proyectoBasedeDatos
{
    public partial class Curso : Form
    {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            switch (opcion)
            {
                ...
                case "Curso":
                    ...
                    txtTipo.SelectedItem = dataGridView1.CurrentRow.Cells["tipo"].Value.ToString();

                    dateTimeInicio.Text = dataGridView1.CurrentRow.Cells["fecha_Inicio"].Value.ToString();
                    dateTimeFin.Text = dataGridView1.CurrentRow.Cells["fecha_Fin"].Value.ToString();
        ...
        private void btnNuevo_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(curso.insertar(int.Parse(txtID_Horario.Text), int.Parse(txtID_Profesor.Text), txtNombre.Text,int.Parse(txtCupo.Text), dateTimeInicio.Text,dateTimeFin.Text,float.Parse(txtCosto_Horas.Text.Replace('.',',')),txtTipo.SelectedItem.ToString()));
                curso.cargaDatos(dataGridView1, opcion);
                this.limpiar();
            }
        }
        ...
        {
            MessageBox.Show(curso.modificar(int.Parse(txtID_Horario.Text), int.Parse(txtID_Profesor.Text), txtNombre.Text, int.Parse(txtCupo.Text), dateTimeInicio.Text, dateTimeFin.Text, float.Parse(txtCosto_Horas.Text.Replace('.', ',')), txtTipo.SelectedItem.ToString(), int.Parse(txtID_Curso.Text)));
            curso.cargaDatos(dataGridView1, opcion);
            this.limpiar();
        }
